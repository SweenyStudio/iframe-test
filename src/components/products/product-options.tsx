    ...
    <div className="grid mt-5 w-full lg:grid-cols-2 gap-3">
      {
        sizing.map((size, index) => (
          <a
            ...
            id={size.variantId}
            className={`flex flex-col items-center w-full rounded-xl py-2 lg:py-3 text-lg cursor-pointer font-bold text-neutral-900 duration-200 ring-0 bg-neutral-50 border border-neutral-300 hover:ring-1 hover:ring-neutral-400 ${selectedVariant.variantId === size.variantId ? 'ring-lime-500 bg-gradient-to-tr from-lime-100 to-green-100' : ''}`}
